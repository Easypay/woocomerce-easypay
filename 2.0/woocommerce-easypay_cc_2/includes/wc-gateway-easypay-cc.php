    ...
    */
    ...
    private function gateway_validation_2()
    {
        if (empty($this->account_id)) {
            add_action('admin_notices', [&$this, 'error_missing_account_id']);
        }
        if (empty($this->api_key)) {
            add_action('admin_notices', [&$this, 'error_missing_api_key']);
        }
        if (!$this->is_valid_for_use()) {
            add_action('admin_notices', [&$this, 'error_invalid_currency']);
        }
        if ($this->expiration_time < 1 || $this->expiration_time > 93) {
            add_action('admin_notices', [&$this, 'error_invalid_expiration']);
        }
    }
